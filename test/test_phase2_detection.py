...
用法:
  ...
  # 指定阶段1输出路径
  python test/test_phase2_detection.py --phase1-output /path/to/phase1_segments.json

  # 限制端到端测试的片段数量
  python test/test_phase2_detection.py --max-segments 5
"""
...
DEFAULT_PHASE1_OUTPUT = os.path.join(PROJECT_ROOT, "test", "test_output", "phase1_segments.json")
...
def test_end_to_end_from_phase1(phase1_output: str, output_dir: str,
    ...
    if not segments:
        print("  [结束] 无片段，请先运行 test_phase1_tag_screening.py")
        return

    if max_segments > 0 and len(segments) > max_segments:
        print(f"  限制测试数量: 取前 {max_segments} 个片段")
    ...
    for i, seg in enumerate(segments):
        ...
        status = "确认" if seg.get('confirmed') else "拒绝"
        reason = seg.get('confirm_reason', '')
        print(f"    检测: [{status}] {reason}")

        if not seg.get('confirmed'):
            continue

        # 合规检查
        print(f"  [3/5] 合规检查...")
        check_compliance(seg, frame_data)
        compliance_passed = seg.get('compliance', {}).get('passed', False)
        ...
        if features is not None:
            ...
            seg['scene_features'] = extract_scene_features(features, seg['scene_type'])
            seg['features'] = features
            print(f"    特征: 通用 {len(seg['general_features'])} 个, "
                  f"场景 {len(seg['scene_features'])} 个")
    ...
    feature_path = write_features(segments, output_dir)

    # 汇总
    confirmed = sum(1 for s in segments if s.get('confirmed', False))
    compliant = sum(1 for s in segments
    ...
    if segments:
        ...
        for seg in segments:
            ...
            confirmed_mark = "Y" if seg.get('confirmed') else "N"
            compliant_mark = "Y" if seg.get('compliance', {}).get('passed') else "N"
            print(f"    {seg.get('dir_key', '?')[:50]}  "
                  f"S{scene_type}({name})  "
...
def main():
    ...
    parser.add_argument("--phase1-output", type=str, default="",
                        help="阶段1输出的片段文件路径 (默认 test/test_output/phase1_segments.json)")
    parser.add_argument("--max-segments", type=int, default=0,
                        help="端到端测试最大片段数 (0=不限制)")
