...
阶段1测试: 标签初筛 + 分组合并 (云平台 batch 目录结构)
...
def test_group_and_merge(batch_dirs, driver_reverse_index, date_split, max_dirs):
    ...
    print(f"  候选片段: {len(all_segments)}")

    if all_segments:
        type_counts = defaultdict(int)
        for seg in all_segments:
            type_counts[seg['scene_type']] += 1
        print(f"\n  按场景类型统计:")
        for st, count in sorted(type_counts.items()):
            print(f"    Scene {st} ({scene_names.get(st, '?')}): {count} 个")

        # 片段明细 (仅展示前 20 个)
        show_limit = min(20, len(all_segments))
        print(f"\n  片段明细 (前 {show_limit}/{len(all_segments)}):")
        for seg in all_segments[:show_limit]:
            name = scene_names.get(seg['scene_type'], f"scene{seg['scene_type']}")
            print(f"\n    [{seg.get('batch', '?')}] {seg['dir_key']}")
            print(f"      Scene {seg['scene_type']} ({name}), "
                  f"driver={seg['driver_id']}")
            print(f"      帧数: {seg['frame_count']}, "
                  f"时长: {seg['duration_sec']:.1f}s")
            print(f"      cloud_path: {seg['cloud_path'][:60]}...")

    return all_segments, merged_stats


def main():
    parser = argparse.ArgumentParser(
        description="阶段1测试: 标签初筛 + 分组合并 (云平台 batch 结构)"
    )
    ...
    # 测试3: 分组 + 合并
    all_segments, stats = test_group_and_merge(
    ...
    phase1_output = os.path.join(output_dir, "phase1_segments.json")
    ...
    print(f"  候选片段:   {len(all_segments)} 个")
    print(f"  片段已保存: {phase1_output} ({len(serializable)} 个)")
    print("  阶段2 测试请运行:")
